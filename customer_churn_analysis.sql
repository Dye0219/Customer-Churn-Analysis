...
--Distribution of senior citizen and churn rate
SELECT 
...
--Churn rate by gender, senior status, partner status, and dependents status
SELECT gender,
       CASE WHEN senior_citizen = 1 THEN 'Yes' ELSE 'No' END AS senior_citizen,
	   partner,
	   dependents,
...
--Combined tech & entertainment services churn analysis
...
--Multi-services relationship with churn rate
...
--Internet services and multi-services relationship with churn rate
WITH multiple_service AS (
  SELECT customer_id, internet_service, churn,
    (
      (CASE WHEN online_security = 'Yes' THEN 1 ELSE 0 END) +
      (CASE WHEN online_backup = 'Yes' THEN 1 ELSE 0 END) +
      (CASE WHEN device_protection = 'Yes' THEN 1 ELSE 0 END) +
...
SELECT num_service, 
       ...
       ROUND(SUM(CASE WHEN churn = 'Yes' THEN 1 ELSE 0 END)::NUMERIC / COUNT(*) * 100, 2) AS churn_rate
FROM multiple_service
GROUP BY num_service,internet_service
ORDER BY num_service ASC;
